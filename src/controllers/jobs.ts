...
import { readJobsFromFile, writeJobsToFile } from '../utils/jobs';
import { Job } from '../types';
import { jobQueue } from '../service/bullmq';
...
export const createJob = async (
  ...
  try {
    ...
    //! Approach 2: Using BullMQ
    //  For scalability, and reliability  + High load concurrency handling we need to use message queue system like BullMQ + Redis combo
    // const delay = Math.floor(Math.random() * (300000 - 5000) + 5000);
    const result = await jobQueue.add(
      'fetch-image-job',
      { jobId, query: 'food' },
      { delay: 5000, removeOnComplete: true, backoff: 2 }
    );

    console.log(`Job added to queue: ${result.id}`); // we can add logger here
